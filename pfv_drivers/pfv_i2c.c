...
uint8_t pfv_I2C_Config(uint8_t i2cx, uint8_t mode, uint8_t speed)
{
	...
	if (i2cx == I2C_Seq1)
	{
		...
		// 初始化GPIO
		GPIO_Initure.Pin = I2C_Dev_Tree[0].m_scl.GPIO_Pinx | I2C_Dev_Tree[0].m_sda.GPIO_Pinx ;
...
uint8_t pfc_I2C_WriteByte(uint8_t i2cx, uint8_t device_addr, uint16_t reg_addr, uint8_t reg_data)
{
	...
	if (i2cx == I2C_Seq1)
	{
		if(HAL_I2C_Mem_Write(I2C_Dev_Tree[0].I2C_Handelx, device_addr, reg_addr, I2C_MEMADD_SIZE_8BIT, &reg_data, 1, 1000) != HAL_OK)
		{
			return I2C_RET_ERROR;
		}
	}
...
uint8_t pfv_I2C_ReadByte(uint8_t i2cx, uint8_t device_addr, uint16_t reg_addr, uint8_t *p_data)
{
	...
	if (i2cx == I2C_Seq1)
	{
		if(HAL_I2C_Mem_Read(I2C_Dev_Tree[0].I2C_Handelx, (device_addr | 0x01), reg_addr, I2C_MEMADD_SIZE_8BIT, p_data, 1, 1000) != HAL_OK)
		{
			return I2C_RET_ERROR;
		}
	}
