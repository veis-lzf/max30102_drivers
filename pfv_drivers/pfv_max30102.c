...
cmd_fail: /* 命令执行失败后，切记发送停止信号，避免影响I2C总线上其他设备 */
	...
	if (I2C_RET_OK == pfc_I2C_WriteByte(g_MAX30102_Dev.m_i2cx, g_MAX30102_Dev.m_address, Register_Address, Word_Data))
	{
...
cmd_fail: /* 命令执行失败后，切记发送停止信号，避免影响I2C总线上其他设备 */
	...
	return 0;
#elif (MAX30102_I2C_MODE == USE_HARDWARE_I2C)
	if (I2C_RET_ERROR == pfv_I2C_ReadByte(g_MAX30102_Dev.m_i2cx, g_MAX30102_Dev.m_address, Register_Address, &data))
		return 0;
...
{
	uint32_t timeout = 0;
	while(!(PPG_READY & max30102_Bus_Read(REG_INTR_STATUS_1)))
	{
...
cmd_fail: /* 命令执行失败后，切记发送停止信号，避免影响I2C总线上其他设备 */
	...
	IIC_Stop();
#elif (MAX30102_I2C_MODE == USE_HARDWARE_I2C)
	if (I2C_RET_ERROR == pfv_I2C_ReadMultByte(g_MAX30102_Dev.m_i2cx,
					g_MAX30102_Dev.m_address, Register_Address,
					&g_MAX30102_Dev.m_sensor_data.m_fifo_data[0], 6))
		return;
#else
	#error "MAX30102_I2C_MODE Config Error!"
#endif
...
#elif (MAX30102_I2C_MODE == USE_HARDWARE_I2C)
	if (I2C_RET_ERROR == pfc_I2C_WriteByte(g_MAX30102_Dev.m_i2cx, g_MAX30102_Dev.m_address, uch_addr, uch_data))
		return;
#else
	#error "MAX30102_I2C_MODE Config Error!"
#endif
...
#elif (MAX30102_I2C_MODE == USE_HARDWARE_I2C)
	if (I2C_RET_ERROR == pfv_I2C_ReadByte(g_MAX30102_Dev.m_i2cx, g_MAX30102_Dev.m_address, uch_addr, puch_data))
		return;
#else
	#error "MAX30102_I2C_MODE Config Error!"
#endif
...
uint8_t max30102_init(void)
{
	...
	if ((max30102_readDeviceID() & 0xff) != DEV_PART_ID)
	{
		...
		return MAX30102_RET_ERROR;
	}
	p_info("max30102 Device ID=0x%04x", max30102_readDeviceID());
