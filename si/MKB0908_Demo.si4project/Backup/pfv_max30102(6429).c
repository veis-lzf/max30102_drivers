...
void max30102_FIFO_ReadWords(uint8_t Register_Address, uint16_t Word_Data[][2], uint8_t count)
{
	...
	while (no)
	{
		if(I2C_RET_ERROR == pfv_I2C_ReadMultByte(g_MAX30102_Dev.m_i2cx, g_MAX30102_Dev.m_address | I2C_RD, Register_Address, (uint8_t *)&Word_Data[i][0], 2))
			return;
		
		if(I2C_RET_ERROR == pfv_I2C_ReadMultByte(g_MAX30102_Dev.m_i2cx, g_MAX30102_Dev.m_address | I2C_RD, Register_Address, (uint8_t *)&Word_Data[i][1], 2))
			return;
		no--;
		i++;
	}
...
cmd_fail: /* 命令执行失败后，切记发送停止信号，避免影响I2C总线上其他设备 */
	...
	IIC_Stop();
#elif (MAX30102_I2C_MODE == USE_HARDWARE_I2C)
	if(I2C_RET_ERROR == pfv_I2C_ReadMultByte(g_MAX30102_Dev.m_i2cx, g_MAX30102_Dev.m_address | I2C_RD, Register_Address, Data, 6))
		return;
#else
	#error "MAX30102_I2C_MODE Config Error!"
#endif
