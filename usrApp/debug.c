...
{
	p_info("info:OnCfgDebug:Type=%ld,P1=%ld,P2=%ld,P3=%ld.", vp_Type, vp_P1, vp_P2, vp_P3);
...
//	p_dbg("nlen=%d", nlen);
	for (i = 0; i < nlen; i++)
	{
		if ((',' == data_buf[i]) && (i < nlen - 1))
			memcpy(vl_FormateStr + strlen(vl_FormateStr), ",%ld", strlen(",%ld"));
	}
//	p_dbg("vl_FormateStr=%s", vl_FormateStr);
	...
	p_dbg("vl_CmdId=%ld, vl_Type=%ld, vl_P1=%ld, vl_P2=%ld, vl_P3=%ld", vl_CmdId, vl_Type, vl_P1, vl_P2, vl_P3);
